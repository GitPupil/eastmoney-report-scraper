...
- **v2.0**：模块化、测试、manifest、质量评分、评分拆解与横向研究输出
- **v1.1**：稳定性增强
- **v1.2**：研究流程化输出
- **v1.3**：单篇结构化分析
- **v1.4**：多篇横向归纳
...
- 包结构重构：`client / parser / analysis / scoring / exporters / cli`
- `run_manifest.jsonl` 运行状态记录
- HTML 结构化解析与文本质量评分
- `scoreReasons` / `scoreBreakdown` 评分透明化导出
- `CONSENSUS_BRIEF.md`
- 区间任务 `RANGE_DASHBOARD.md`
- `--refresh-weak`、`--resume-errors-only`、`--min-text-length`、`--jitter`、`--manifest-name`
- pytest 回归测试与 ruff 开发依赖
...
- 详情页失败自动重试
- `run.log.jsonl` 结构化日志
- 已有 markdown 与 `run_manifest.jsonl` 默认断点续跑
- 支持 `--force` 强制重抓
...
- 当前结构化分析仍为规则驱动，适合研究初筛和归档，不等同于投资建议
