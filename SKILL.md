...
- Local App 提供可选 AI 分析接口；只有用户明确要求在本地 App 中配置或调用 AI 分析时，才使用 `/api/ai/settings`、`/api/ai/evidence`、`/api/ai/analyze`、`/api/ai/batch`、`/api/ai/compare`、`/api/ai/history` 或页面里的“AI 分析”面板。OpenClaw 默认仍读取结构化输出自行分析。
